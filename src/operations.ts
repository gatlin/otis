...
class List implements Operation {
  constructor(public readonly ops: Operation[]) {}
...
class Splice implements Operation {
  ...
  public compose(other: Operation): Operation | null {
    ...
    if (other instanceof Splice) {
      if (
        this.pos <= other.pos &&
        other.pos + other.old_value.length <= this.pos + this.new_value.length
      ) {
        const item1: string | Value[] = this.new_value.slice(
          0,
          other.pos - this.pos
        );
        ...
        return new Splice(this.pos, this.old_value, catted);
      }

      if (
        other.pos <= this.pos &&
        this.pos + this.new_value.length <= other.pos + other.old_value.length
      ) {
        const item1: string | Value[] = other.old_value.slice(
          0,
          this.pos - other.pos
        );
        ...
        const catted: typeof item3 = concat3(item1, item2, item3);
        return new Splice(other.pos, catted, other.new_value);
      }
    }
    if (other instanceof Set) {
      return new Set(
    ...
    if (other instanceof ArrayApply) {
      if (
        other.pos >= this.pos &&
        other.pos < this.pos + this.old_value.length
      ) {
        return new Splice(
          this.pos,
          this.old_value,
  ...
  private rebase_map(other: Map): [Operation, Operation] | null {
    try {
      ...
      return [new Splice(this.pos, _old_value, _new_value), other];
    } catch (err) {
      const __old_value: unknown = out(other.apply(represent(this.old_value)));
      const _old_value: string | Value[] = __old_value as string | Value[];
      return [new Splice(this.pos, _old_value, this.new_value), new NoOp()];
    }
    return null;
  }
  ...
  private rebase_arrayapply(other: ArrayApply): [Operation, Operation] | null {
    ...
    const old_value = concat3(
      ...
      unelem(
        other.op.apply(elem(this.old_value, other.pos - this.pos)),
        this.old_value
      ),
    ...
    if (this.new_value.length === this.old_value.length) {
      const new_value = concat3(
        ...
        unelem(
          other.op.apply(elem(this.new_value, other.pos - this.pos)),
          this.old_value
        ),
        this.new_value.slice(other.pos - this.pos + 1)
      );
      return [new Splice(this.pos, old_value, new_value), other];
    }
    return [new Splice(this.pos, old_value, this.new_value), new NoOp()];
  }
  ...
  private rebase_splice(other: Splice): [Operation, Operation] | null {
    ...
    // overlapping splices
      ...
      const item1: typeof _ov = this.old_value.slice(0, other.pos - this.pos);
      const item3: typeof item1 = this.old_value.slice(
        other.pos + other.old_value.length - this.pos
      );
      const catted: typeof item3 = concat3(item1, other.new_value, item3);
      // we absorb their splice into ours
      return [new Splice(this.pos, catted, this.new_value), new NoOp()];
    }
...
class Insert extends Splice {
  constructor(pos: number, value: string | Value[]) {
...
class Delete extends Splice {
  constructor(pos: number, old_value: string | Value[]) {
...
class Move implements Operation {
  ...
  public apply(value: Value): Value {
    ...
    if (this.pos < this.new_pos) {
      ...
      const item2: typeof item1 = _value.slice(
        this.pos + this.count,
        this.new_pos
      );
      const item3_a: typeof item2 = _value.slice(
        this.pos,
        this.pos + this.count
      );
  ...
  public rebase(other: Operation): [Operation, Operation] | null {
    ...
    if (other instanceof ArrayApply) {
      return [this, new ArrayApply(map_index(other.pos, this), other.op)];
    }
    if (other instanceof Map) {
      return [this, other];
    }
...
class Set implements Operation {
  ...
  constructor(old_value: unknown, new_value?: unknown) {
  ...
  public rebase(other: Operation): [Operation, Operation] | null {
    ...
    if (other instanceof Set) {
      ...
      if (this.new_value && cmp(this.new_value, other.new_value) < 0) {
        return [new NoOp(), new Set(this.new_value as Value, other.new_value)];
      }
    }
    return null;
  }
...
class ArrayApply implements Operation {
  constructor(public readonly pos: number, public readonly op: Operation) {}
  ...
  public rebase(other: Operation): [Operation, Operation] | null {
    ...
    if (other instanceof ArrayApply) {
      ...
      if (opa && opb) {
        return [
          opa instanceof NoOp ? new NoOp() : new ArrayApply(this.pos, opa[0]),
          opb instanceof NoOp ? new NoOp() : new ArrayApply(other.pos, opb[1])
        ];
      }
    }
    if (other instanceof Map) {
      ...
      if (opa && opb && deepEqual(opa[0], opb[1])) {
        ...
        return [r, new List([this.invert(), other, r]).simplify()];
      }
    }
    return null;
  }
  public invert(): Operation {
    return new ArrayApply(this.pos, this.op.invert());
  }
  public compose(other: Operation): Operation | null {
    if (other instanceof NoOp) {
      return this;
      ...
      if (
        ...
        return new Splice(
          ...
          concat3(
            ...
            unelem(
              this.invert().apply(elem(other.old_value, this.pos - other.pos)),
              other.old_value
            ),
...
class Map implements Operation {
  ...
  public apply(value: Value): Value {
    ...
    const _value: string | Value[] = __value as string | Value[];
    const d: Value[] =
      "string" === typeof _value
        ? (_value.split(/.{0}/).map(atom) as Value[])
        : _value.slice();
    for (let i = 0; i < d.length; i++) {
      d[i] = this.op.apply(d[i]);
    }
    return "string" === typeof _value ? atom(d.join("")) : arr(d);
  }
...
class Put implements Operation {
  constructor(public readonly key: string, public readonly value: Value) {}
...
class Remove implements Operation {
  constructor(public readonly key: string, public readonly old_value: Value) {}
  ...
  public rebase(other: Operation): [Operation, Operation] | null {
    ...
    if (other instanceof Rename) {
      if (this.key === other.old_key) {
        return [new Remove(other.new_key, this.old_value), new NoOp()];
      }
      return [this, other];
    }
    if (other instanceof ObjectApply) {
  ...
  public rebase(other: Operation): [Operation, Operation] | null {
    ...
    if (other instanceof Rename) {
      if (this.old_key === other.old_key) {
        ...
        if (cmp(this.new_key, other.new_key) < 0) {
          return [new NoOp(), new Rename(this.new_key, other.new_key)];
        }
        return null;
      }
      if (this.new_key === other.new_key) {
    ...
    if (other instanceof ObjectApply) {
      if (this.old_key === other.key) {
        return [this, new ObjectApply(this.new_key, other.op)];
      }
      return [this, other];
    }
    return null;
...
class ObjectApply implements Operation {
  constructor(public readonly key: string, public readonly op: Operation) {}
